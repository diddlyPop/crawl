...
static formatted_string _choose_name_prompt(const newgame_def& ng, const char *name,
        bool overwrite_prompt, bool good_name)
{
    formatted_string prompt;
    string specs = chop_string(species_name(ng.species), 79, false);
    prompt.cprintf("You are a%s %s %s.\n",
...
static void _choose_name(newgame_def& ng, newgame_def& choice)
{
    ...
    prompt_ui->on(UI::slots.event, [&](wm_event ev)  {
        ...
        prompt_ui->set_text(_choose_name_prompt(ng, buf, overwrite_prompt, good_name));
...
UISizeReq UINewGameMenu::_get_preferred_size(int dim, int prosp_width)
{
    UISizeReq ret;
    if (!dim)
        ret = { 80, 80 };
...
bool UINewGameMenu::on_event(wm_event ev)
{
#ifdef USE_TILE_LOCAL
                        ...
                        const newgame_def& defaults)
{
    ...
    auto popup = make_shared<UIPopup>(newgame_ui);
...
class UIPrecisionMenuWrapper : public UI
{
public:
...
UISizeReq UIPrecisionMenuWrapper::_get_preferred_size(int dim, int prosp_width)
{
    UISizeReq ret;
    if (!dim)
        ret = { w, w };
...
bool UIPrecisionMenuWrapper::on_event(wm_event ev)
{
#ifdef USE_TILE_LOCAL
    if (ev.type == WME_MOUSEMOTION
     || ev.type == WME_MOUSEBUTTONDOWN
