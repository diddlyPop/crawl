...
UISizeReq UIDollEditor::_get_preferred_size(int dim, int prosp_width)
{
    if (!dim)
        return { 552, 552 };
    else
...
void DollEditRegion::run()
{
    ...
    auto title = make_shared<UIText>("<yellow>Doll Editor</yellow>");
    ...
    ui_push_layout(move(popup), KMC_DOLL);
    while (!done)
    {
        if (update_part_idx)
        {
